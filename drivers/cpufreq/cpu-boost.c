...
static inline void *get_cpu_sync_info(u32 cpu)
{
...
static int get_input_boost_freq(char *buf, const struct kernel_param *kp)
{
	...
	for_each_possible_cpu(cpu) {
		s = get_cpu_sync_info(cpu);
		len += scnprintf(buf + len, 27,
				 cpu != (num_possible_cpus() - 1) ? "%d:%u " :
...
static void do_input_boost_rem(struct work_struct *work)
{
	...
	for_each_possible_cpu(cpu) {
		i_sync_info = get_cpu_sync_info(cpu);
